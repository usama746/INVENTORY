...
function renderItemCard(item) {
  ...
  table.style.overflowX = isItemEditing ? "auto" : "visible";
  table.innerHTML = `
    <table style="width: 100%; table-layout: auto;">
      <thead>
        <tr>
          <th style="text-align: center; padding: 4px 2px;">Color</th>
          <th style="text-align: center; padding: 4px 2px;">Price</th>
          <th style="text-align: center; padding: 4px 2px;">Qty</th>
          ${isItemEditing ? '<th style="text-align: center; padding: 4px 2px; width: 90px;">Actions</th>' : ""}
        </tr>
      </thead>
      <tbody></tbody>
    </table>
  ...
  if (isItemEditing) {
    tr.innerHTML = `<td colspan="4">
      <div class="add-variant-row" style="text-align: center; margin: 4px 8px; vertical-align: middle; display: flex; align-items: center; justify-content: center;">
        <button class="btn add-variant-btn w-100" style="font-weight: 400;">
...
function renderVariantRow(item, variant, tbody) {
  ...
  if (isItemEditing) {
    tr.innerHTML = `
      <td style="padding: 4px 2px; text-align: center; vertical-align: middle;">
        <input id="edit-variant-color-${variant.id}" type="text" value="${variant.color}" style="display: inline-block; min-width: 80; max-width: 100px; border: 1px solid var(--border); border-radius: 4px; padding: 4px 10px ; font-size: 14px; ">
      </td>
      <td style="text-align: center; padding: 4px 2px;">
        <input id="edit-variant-price-${variant.id}" type="number" min="0" value="${variant.price}" style=" min-width: 80; max-width: 100px; border: 1px solid var(--border); border-radius: 4px; padding: 4px 10px; font-size: 14px; text-align: center;">
      </td>
      <td style="text-align: center; padding: 4px 2px;">
        <input id="edit-variant-qty-${variant.id}" type="number" min="0" value="${variant.qty}" style="min-width: 40; max-width: 60px; border: 1px solid var(--border); border-radius: 4px; padding: 4px 10px; font-size: 14px; text-align: center;">
      </td>
      <td style="text-align: center; padding: 4px 2px;">
    ...
    tr.innerHTML = `
      <td style="font-weight: 500; padding: 4px 2px; text-align: center;">${variant.color}</td>
      <td style="text-align: center; padding: 4px 2px; font-family: 'SF Mono', Monaco, 'Cascadia Code', 'Roboto Mono', Consolas, 'Courier New', monospace; font-weight: 500;">${formatPrice(variant.price)}</td>
      <td style="text-align: center; padding: 4px 2px; font-family: 'SF Mono', Monaco, 'Cascadia Code', 'Roboto Mono', Consolas, 'Courier New', monospace; font-weight: 500;">${variant.qty}</td>
    `;
  }
  tbody.appendChild(tr);
}
...
function startAddVariant(itemId) {
  ...
  if (addVariantRow) {
    addVariantRow.innerHTML = `
      <td style="padding:  4px 2px; text-align: left; vertical-align: middle;">
        <input id="new-variant-color" type="text" placeholder="Color" style="display: inline-block; min-width: 80; max-width: 100px; border: 1px solid var(--border); border-radius: 4px; padding: 4px 10px; font-size: 14px;">
      </td>
      <td style="text-align: start; padding:  4px 2px;">
        <input id="new-variant-price" type="number" min="0" placeholder="Price" style="min-width: 80; max-width: 100px; border: 1px solid var(--border); border-radius: 4px; padding: 4px 10px; font-size: 14px; text-align: start;">
      </td>
      <td style="text-align: start; padding:  4px 2px;">
        <input id="new-variant-qty" type="number" min="0" placeholder="Qty" style="min-width: 40; max-width: 60px; border: 1px solid var(--border); border-radius: 4px; padding: 4px 10px; font-size: 14px; text-align: start;">
      </td>
      <td style="text-align: center; padding:  4px 2px;">
